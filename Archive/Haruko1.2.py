...
def get_chatbot_response():
    ...
    if "name" in user_input.lower() and "your" in user_input.lower():
        chatbot_response = "My name is Haruko."
...
#               **** Failed to implement this without severe bugs and her unable to ask other questions without answering Seth. 
#               **** Pre determined responses should be at a minimum to prevent interference with Blenderbot Model
#             # Check if user asked who the admin is
#            if ("who" in user_input.lower() and "admin" in user_input.lower() and "creator" in user_input.lower()):
#                chatbot_response = "Seth Bates"
#                response = None  # Assigning a default value to response
#            else:


                # Check if user asked who is a specific person
                if "who is" in user_input.lower():
                    # Encode the user input
                    inputs = tokenizer([user_input], return_tensors="pt")

                    # Generate a response from the model with a random temperature
                    temperature = random.uniform(0.7, 1.0)
                    response = model.generate(inputs["input_ids"], max_length=1000, temperature=temperature)[0]
                    ...
                    response = model.generate(inputs["input_ids"], max_length=1000)[0]
